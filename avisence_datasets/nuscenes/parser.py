...
from utils.kitti_utils import load_calib, load_lidar_token, load_poses
...
class SemanticKitti(Dataset):
    ...
    def __init__(
        ...
        for seq in self.sequences:
            ...
            if self.use_residual:
                for i in range(self.n_input_scans):
                    exec(
                        "residual_files_" + str(i + 1) + " = " + "[os.path.join(dp, f) for dp, dn, fn in "
    ...
    def __getitem__(self, dataset_index):
        ...
        # 	if random.random() > 0.5:
        #       cutline = random.randint(self.sensor_img_W / 4, self.sensor_img_W * 3 / 4)
        #       proj_full = torch.cat((proj_full[:, :, cutline:], proj_full[:, :, :cutline]), dim=2)
        #       proj_mask = torch.cat((proj_mask[:, cutline:], proj_mask[:, :cutline]), dim=1)
        #       proj_labels = torch.cat((proj_labels[:, cutline:], proj_labels[:, :cutline]), dim=1)
        # 	if random.random() > 0.5:
    ...
    def remove_few_static_frames(self):
        ...
        remove_mapping_path = os.path.join(
            os.path.dirname(__file__), "../../../config/train_split_dynamic_pointnumber.txt"
        )
