...
class SemanticKitti(Dataset):
    ...
    def __init__(
        ...
        for seq in self.sequences:
            ...
            # get files
            scan_files = [
                os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser(scan_path)) for f in fn if is_scan(f)
            ]
            tag_files = [
                os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser(tag_path)) for f in fn if is_tag(f)
            ]
            label_files = [
                os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser(label_path)) for f in fn if is_label(f)
            ]
    ...
    def __getitem__(self, index):
        ...
        proj = torch.cat(
            [proj_range.unsqueeze(0).clone(), proj_xyz.clone().permute(2, 0, 1), proj_remission.unsqueeze(0).clone()]
        )
...
class Parser:
    ...
    def __init__(
        ...
        self.validloader = torch.utils.data.DataLoader(
            self.valid_dataset, batch_size=self.batch_size, shuffle=False, num_workers=self.workers, drop_last=True
        )
        ...
        if self.test_sequences:
            ...
            self.testloader = torch.utils.data.DataLoader(
                self.test_dataset, batch_size=self.batch_size, shuffle=False, num_workers=self.workers, drop_last=True
            )
