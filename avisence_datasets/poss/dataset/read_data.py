...
def get_rangeimage(bin_file, tag_file):
    ...
    tags = np.fromfile(tag_file, dtype=bool)
...
if __name__ == "__main__":  # an example for generating video of sequence 00
    ...
    for bin_file, label_file, tag_file in zip(
        sorted(os.listdir(data_path + "velodyne/")),
        sorted(os.listdir(data_path + "labels/")),
        sorted(os.listdir(data_path + "tag/")),
    ):
