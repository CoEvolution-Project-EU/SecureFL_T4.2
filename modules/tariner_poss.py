...
from modules.utils import AverageMeter, save_to_txtlog
...
class TrainerPoss(Trainer):
        ...
        for i, (in_vol, proj_labels, _, _, path_seq, path_name, _, _, _, _, _) in enumerate(train_loader):
            ...
            if self.ARCH["train"]["aux_loss"]["use"]:
                ...
                bdlosss = (
                    ...
                    + lamda[0] * self.bd(z2, proj_labels.long())
                    + lamda[1] * self.bd(z4, proj_labels.long())
                    + lamda[2] * self.bd(z8, proj_labels.long())
                )
                loss_m0 = criterion(torch.log(output.clamp(min=1e-8)).double(), proj_labels).float() + 1.5 * self.ls(
                    output, proj_labels.long()
                )
                loss_m2 = criterion(torch.log(z2.clamp(min=1e-8)).double(), proj_labels).float() + 1.5 * self.ls(
                    z2, proj_labels.long()
                )
                loss_m4 = criterion(torch.log(z4.clamp(min=1e-8)).double(), proj_labels).float() + 1.5 * self.ls(
                    z4, proj_labels.long()
                )
                loss_m8 = criterion(torch.log(z8.clamp(min=1e-8)).double(), proj_labels).float() + 1.5 * self.ls(
                    z8, proj_labels.long()
                )
            ...
            if i % self.ARCH["train"]["report_batch"] == 0:
                str_line = (
                    ...
                    "IoU {iou.val:.3f} ({iou.avg:.3f}) | [{estim}]"
                ).format(
                    epoch,
                    i,
                    len(train_loader),
    ...
    def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans=False):
        ...
        with torch.no_grad():
            ...
            for i, (in_vol, proj_labels, _, _, path_seq, path_name, _, _, _, _, _) in enumerate(
                tqdm(val_loader, desc="Validation:", ncols=80)
            ):
                if not self.multi_gpu and self.gpu:
                    in_vol = in_vol.cuda()
                if self.gpu:
            ...
            str_line = (
                ...
                "Acc avg {acc.avg:.6f}\n"
                "IoU avg {iou.avg:.6f}"
            ).format(batch_time=self.batch_time_e, loss=losses, jac=jaccs, wces=wces, acc=acc, iou=iou)
            print(str_line)
            save_to_txtlog(self.logdir, "log.txt", str_line)
            ...
            for i, jacc in enumerate(class_jaccard):
                ...
                str_line = "IoU class {i:} [{class_str:}] = {jacc:.6f}".format(i=i, class_str=class_func(i), jacc=jacc)
                print(str_line)
                save_to_txtlog(self.logdir, "log.txt", str_line)
            str_line = "*" * 80
            print(str_line)
    ...
    def make_log_img(depth, pred, gt, color_fn):
        ...
        depth = (
            cv2.normalize(depth, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F) * 255.0
        ).astype(np.uint8)
        out_img = cv2.applyColorMap(depth, Trainer.get_mpl_colormap("viridis"))
