...
def save_checkpoint(to_save, logdir, pipeline="LENet", suffix=""):
    # Save the weights
    torch.save(to_save, logdir + "/" + pipeline + suffix)
...
def make_log_img(depth, mask, pred, gt, color_fn):
    ...
    depth = (cv2.normalize(depth, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F) * 255.0).astype(
        np.uint8
    )
...
class iouEval:
    ...
    def getacc(self):
        ...
        print(f"total_tp: {total_tp} total: {total}")
