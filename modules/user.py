...
class User:
    def __init__(self, ARCH, DATA, datadir, outputdir, modeldir, split, point_refine=False):
        ...
        # get the data
        imp.load_source("parserModule", f"{booger.TRAIN_PATH}/dataset/{self.DATA['name']}/parser.py")
        self.parser = Parser(
            root=self.datadir,
    ...
    def infer(self):
        ...
        if self.split == "valid":
            self.infer_subset(loader=self.parser.get_valid_set(), to_orig_fn=self.parser.to_original, cnn=cnn, knn=knn)
        elif self.split == "train":
            self.infer_subset(loader=self.parser.get_train_set(), to_orig_fn=self.parser.to_original, cnn=cnn, knn=knn)
